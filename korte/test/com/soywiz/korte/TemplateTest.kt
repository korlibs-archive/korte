...
class TemplateTest : BaseTest() {
	...
	@Test fun testMerge() = syncTest {
		Assert.assertEquals("[1, 2, 3, 4]", Template("{{ [1, 2]|merge([3, 4]) }}")(null))
	}
	...
	@Test fun testComment() = syncTest {
		Assert.assertEquals("a", Template("{# {{ 1 }} #}a{# #}")(null))
	}
